...
async def photo_state(message: types.Message, state: FSMContext):
    if message.text != None and len(message.text) < 25:
        ...
        await bot.send_message(message.chat.id, 'Выберете фото для вашей анкеты или напишите любое сообщение чтобы продолжить без фото', reply_markup=next_back_kb_markup)
    else:
...
async def new_profile(call: types.callback_query):
    ...
    try:
        ...
        try:
            await bot.send_photo(call.from_user.id, result[count]['photo'],  caption=x, reply_markup=search_profile_markup)
        except Exception as e:
            get_log_errors(e)
    ...
    try:
        ...
        if us_name in list_liked_users(like_user_name):
            ...
            newprofile1 = (f'Имя: {result[count]["nick_name"]}'
                          ...
                          f'Возраст: {result[count]["age"]}\n'
                          f'О себе: {result[count]["about_me"]}')
            await bot.send_photo(call.from_user.id, photo=result[count]['photo'], caption=newprofile1, reply_markup=search_profile_markup)
        # except TypeError:
        #     photos = "AgACAgIAAxkBAAIFe2UfsirjHLkRHmqocmNSZIphy4FfAAKXzjEb748BSeSusLw4RhVIAQADAgADeAADMAQ"
        #     await bot.send_photo(call.from_user.id, photos, caption=newprofile1, reply_markup=search_profile_markup)

    except IndexError:
        await bot.send_message(call.from_user.id, 'На сегодня анкеты закончились. Хотите пообщаться в нашем чате?', reply_markup=again_markup)
...
async def like_not(call: types.callback_query):
    ...
    try:
        ...
        await bot.send_message(call.from_user.id, 'На сегодня анкеты закончились. Хотите пообщаться в нашме чате?', reply_markup=again_markup)
...
async def try_name(message: types.Message, state:FSMContext):
    if message.text != None and len(message.text) < 25:
        ...
        await state.clear()
        await bot.send_message(message.chat.id, "Введите имя буквами и цифрами до 25 символов", reply_markup=edit_pro_markup)
...
def register_handlers(dp: Dispatcher):
    """******************ADMIN_1*********************"""
    dp.message.register(admin, Command('admin1213'))
    dp.message.register(ad_key, F.text == '124ffsf')
    dp.callback_query.register(mess_all, F.data == 'send_all')
    dp.message.register(image_all, Fsm1.message_all)
    dp.message.register(res, Fsm1.image_al)
    dp.callback_query.register(sends_all, Fsm1.resl)

    """меняем"""
    dp.callback_query.register(edit_pref_age, F.data == 'pref_age')
    dp.callback_query.register(try_pref_age, FSMage.agestep_1)
    dp.message.register(my_chat, F.text == 'Перейти в наш чат')
    dp.message.register(help1, F.text == 'Помощь')
    dp.callback_query.register(try_city, Fsmcity.editcity)
    dp.callback_query.register(edit_city, F.data == 'edit_city')
    dp.callback_query.register(edit_perf, F.data == 'edit_perf')
    dp.callback_query.register(try_perf, Fsmperf.editperf)
    dp.callback_query.register(edit_about, F.data == 'edit_about')
    dp.message.register(try_about, Fsmabout.editabout)
    dp.callback_query.register(edit_photo, F.data == 'edit_photo')
    dp.message.register(try_photo, Fsmphoto.editphoto)
    dp.callback_query.register(try_gender, Fsmeditgender.editgender)
    dp.callback_query.register(edit_gender, F.data == 'edit_gender')
    dp.callback_query.register(edit_age, F.data == 'edit_age')
    dp.message.register(try_age, Fsmeditage.editage)
    dp.callback_query.register(edit_name, F.data == 'edit_name')
    dp.message.register(try_name, Fsmeditname.fname)
    dp.callback_query.register(back, F.data == 'cancell')
    """обновление поиска"""
    dp.callback_query.register(profile_again, F.data == 'go_again')
    """КОМАНДЫ"""
    dp.message.register(cmd_start, Command('start'))
    dp.message.register(cmd_my_profile, Command('my_profile'))
    """SEARCH"""
    dp.callback_query.register(like_not, F.data == 'dislike')
    dp.callback_query.register(like_do, F.data == 'like')
    dp.callback_query.register(new_profile, F.data == 'go')
    """********************Анкета***********************"""
    dp.callback_query.register(edit_pro, F.data == 'edit_profile')
    dp.callback_query.register(edit_pro_1, F.data == 'edit_1_profile')
    dp.callback_query.register(search_city, FSMprofile.search_profile)
    dp.callback_query.register(cancel, F.data == 'back')
    dp.message.register(search_state, FSMprofile.age)
    dp.callback_query.register(name_state, FSMprofile.sex)
    dp.message.register(photo_state, FSMprofile.name)
    dp.message.register(about_state, FSMprofile.photo)
    dp.callback_query.register(ready_profile, FSMprofile.age_seach_s)
    dp.callback_query.register(age_seach, FSMprofile.city)
    dp.message.register(age_state, FSMprofile.text_profile)
    dp.callback_query.register(send_profile, F.data == 'hi')